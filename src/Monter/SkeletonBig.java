...
public class SkeletonBig extends Monster{
	//DrawCircle DC;
	final int thuong=0;
	final int tancong=1;
	final int chet=2;
	...
	int effTime=50;
	int time=0;
	public SkeletonBig(Bitmap bit, float bx, float by, Handler h) {
		super(bit,ID.skeletonBig, bx, by, SubSetting.ruler, 8, 4, h);
		this.setInfo(800, 10, 3, 10);
	...
	public void checkStatup(){
		...
		case thuong:
			...
			if(crImage>=6) crImage=0;
			wailtime=12;
			break;
		case tancong:
			if(Left) rowImage=3;
			else rowImage=2;
			if(crImage<8) crImage=crImage+1;
			if(crImage>=8) crImage=0;
			wailtime=16;
			break;
		case chet:
			if(Left) rowImage=0;
	...
	public Monster makeLone() {
		return new SkeletonBig(bitmap, x, y, handler);
	}
	@Override
	public void draw(Canvas canvas) {
		int k=hp*100/maxhp;
		if(Left) canvas.drawBitmap(drawbitmap, SubSetting.x+x-width/2, SubSetting.y+y-height/2, null);
		else{
			 canvas.drawBitmap(drawbitmap, SubSetting.x+x-width/2+ruler, SubSetting.y+y-height/2, null);
		}
		canvas.drawLine(SubSetting.x+x-width/4,SubSetting.y+y+height/2, SubSetting.x+x-width/2+(width*k/100), SubSetting.y+y+height/2, pa);
		for(int i=0; i<10; i++){
			if(GE[i]!=null){
				//canvas.drawBitmap(GE[i].Drawimg, SubSetting.x+x+width/2-GE[i].width/2, SubSetting.y+y-GE[i].height/2, null);
				canvas.drawBitmap(GE[i].Drawimg, SubSetting.x+x+ruler/2-GE[i].width/2, SubSetting.y+y-GE[i].height/2, null);
			}
		}
	}
}
