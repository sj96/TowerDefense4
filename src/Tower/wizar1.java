...
public class wizar1 extends Tower{
	Bitmap castWi, Fireball, boom;
	RedyCast cast;
	public wizar1(Bitmap bit,Bitmap cast, Bitmap fireb, Bitmap Boom, float bx, float by,Handler h , Handler EnemyList) {
		super(bit, ID.wizar, bx, by, 21, 2, h, EnemyList);
		this.setInfo(120,  15, 4, ruler*3, 4);
		// hp, at, df, rg, bp
		this.blockCount=0;
		castWi=cast;
		Fireball = fireb;
	...
	public Tower MakeClone() {
		wizar1 ac = new wizar1(bitmap, castWi, Fireball, boom, x, y, handler, enemylist);
		return ac;
	}
	...
	public void update() {
		...
		if(time==0 && crImage==18 && block!=null){//tai vi tri anh thu 6
			tancong();
		}
